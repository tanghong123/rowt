...
| **3** ◑ | watchdog: FSM into core, effects via `PlatformMac`; `cmd_watch` execs the Rust tick | **FSM done** — `rowt-core::watch`, 17 unit tests replaying §11's decision table; `parity watch-diff` compares bash's effects against the planned actions, 5/5. Remaining: `PlatformMac` effects, the `cmd_watch` cutover, and the shadow window |
