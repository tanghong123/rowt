...
/// Generation of the plist this rowt writes — the shell's WATCH_PLIST_VERSION.
/// BUMP IT whenever the generated plist changes, so an agent installed by an
...
pub fn plist_stale(plist: &Path) -> bool {
    plist.is_file() && !crate::read(plist).contains(PLIST_MARK)
}
...
fn plist_body(ctx: &Ctx, self_bin: &Path) -> String {
    let rowt_ver = env!("ROWT_SHELL_VERSION");
...
<plist version="1.0">
<!-- {PLIST_MARK} — generated by rowt {rowt_ver}; 'rowt watch install' rewrites it.
...
pub fn cmd(ctx: &Ctx, self_bin: &Path, action: &str) -> Result<String, String> {
    ...
    match action {
        ...
        "status" => {
            ...
            // A plist from an older rowt is the failure that hides: the agent
            // loads and reports LOADED while missing settings this version needs.
            if plist_stale(&plist) {
                o.push(format!("  version: STALE — written by an older rowt (no '{PLIST_MARK}')."));
                o.push(format!("           A tick re-syncs it automatically; to do it now: {PROG} watch install"));
            }
